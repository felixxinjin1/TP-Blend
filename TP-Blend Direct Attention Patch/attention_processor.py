...
class AttnProcessor:
    def __init__(self, enable_CAOF=True, enable_SASF=True, sasf_mode="DSIN", use_ot=True): #AdaIN, DSIN
    ...
    def SASF(
        ...
        """
        Self-Attention Style Fusion function.

        If key,value are None => apply style normalization to hidden_states.
        If key,value exist => do the K/V swap logic.

        Two style modes:
          - "adain": your original approach
          - "FAIN": do AdaIN but also inject fine detail from sty_feat.
        """
        eps = 1e-5
        ...
        def local_smoothing_1d(feat: torch.Tensor, kernel_size, sigma) -> torch.Tensor:
            """
            feat: [tokens, channels]
            Return the smoothed 'feat' by applying a 1D convolution in the token dimension
            (with channels treated as separate signal dimensions).
            """
            # Construct small 1D Gaussian kernel or simple box filter
            # Example: a tiny 5-element Gaussian
            # (Alternatively, a simple uniform kernel = 1/5.0 also works well, but Gaussian is usually sharper.)
            ...
            # Shape: [1, 1, kernel_size] so we can apply depthwise conv over tokens
            # We'll treat channels as separate features => group=channels
            kernel_1d = gauss.view(1, 1, -1)

            # We want to convolve each channel separately => group = #channels
            # But 'feat' is [tokens, channels], so we transpose to [batch=1, channels=?, length=tokens]
            feat = feat.unsqueeze(0).transpose(1, 2)  # => [1, channels, tokens]

            channels = feat.shape[1]
            # Expand the kernel to shape [channels, 1, kernel_size]
            kernel_1d = kernel_1d.expand(channels, 1, kernel_size)

            # Depthwise conv with group=channels
            # Pad so we don't shrink the sequence
            padding = (kernel_size - 1) // 2
            smoothed = F.conv1d(
                feat, kernel_1d, bias=None, stride=1, padding=padding, groups=channels
            )
            # shape => [1, channels, tokens]
            smoothed = smoothed.transpose(1, 2).squeeze(0)  # => [tokens, channels]
            return smoothed

        def inject_details(adain_out, cnt_feat, sty_feat, alpha):
            # 1) local smoothing
            ...
            # 2) Weighted injection
            return adain_out + alpha * (styleHF - contentHF)

        # Decide if we are doing style-blending (key,value=None) or the K/V swap:
        if key is None and value is None:
            # The usual pattern: hidden_states[2,:] = content, hidden_states[4,:] = style
            cnt_feat = hidden_states[2, :, :]
            sty_feat = hidden_states[4, :, :]

            if self.sasf_mode == "AdaIN":
                # 1) Original AdaIN
                hidden_states[2, :, :] = adain(cnt_feat, sty_feat)

            elif self.sasf_mode == "DSIN":
                # 1) AdaIN
            ...
            # Swap K V region. We'll leave it as-is from your original snippet:
    ...
    def CAOF(
        ...
        attention_probs_target = attention_probs[
            indices['target'] * attn.heads
            : (indices['target'] + 1) * attn.heads,
            :, :
        ]

        attention_probs_blend = attention_probs[
            indices['blend'] * attn.heads
            : (indices['blend'] + 1) * attn.heads,
            :, :
        ]
        ...
        attention_target_to_target = attention_probs_target[
            :, :, target_token_index
        ].mean(dim=0)

        attention_blend_to_blend = attention_probs_blend[
            :, :, blend_token_index
        ].mean(dim=0)

        source_threshold = torch.quantile(
            attention_blend_to_blend, source_percentile / 100.0
        )
        dest_threshold = torch.quantile(
            attention_target_to_target, dest_percentile / 100.0
        )
        ...
        hidden_states_target = hidden_states[
            indices['target'], :, :
        ]
        hidden_states_blend = hidden_states[
            indices['blend'], :, :
        ]

        source_embeddings = hidden_states_blend[
            source_indices, :
        ]
        dest_embeddings = hidden_states_target[
            dest_indices, :
        ]

        # ───────────────────────────────────────────────────────────────────
        # NoneOT ablation: simple index‑based α‑blend, no cost matrix / Sinkhorn
        # ───────────────────────────────────────────────────────────────────
        if not self.use_ot:
            m, n = source_embeddings.size(0), dest_embeddings.size(0)
            k = min(m, n)

            # Pair the first k source / dest indices 1‑to‑1.
            # If sets differ in size, leave the unmatched dest positions unchanged.
            blended = dest_embeddings.clone()
            blended[:k] = (1 - w0) * dest_embeddings[:k] + w0 * source_embeddings[:k]
        ...
        spatial_distance = torch.cdist(
            dest_positions, source_positions, p=2
        )

        cost_matrix = lambda_feature * feature_distance + \
                      lambda_spatial * spatial_distance
        cost_matrix = cost_matrix / cost_matrix.max()

        supply = torch.full((m,), 1.0 / m, device=device)
        demand = torch.full((n,), 1.0 / n, device=device)

        transport_plan = self.sinkhorn(
            supply, demand, cost_matrix, gamma, max_iter=1000, tol=1e-6
        )
        ...
        blended_embeddings = (1 - w0) * dest_embeddings + \
                             w0 * torch.mm(transport_plan, source_embeddings)
    ...
    def sinkhorn(self, a, b, C, gamma, max_iter=1000, tol=1e-6):
        K = torch.exp(-C / gamma)
        u = torch.ones_like(a)
        v = torch.ones_like(b)
